...
### 2026-08-20: Anticipo RP saltaba a 66M+ en la proyección — el split RG/RP tenía el ancla al revés

- **Pedido/hallazgo del usuario**, viendo la tabla en vivo: "los anticipos del RP... aumentan sin lógica en la proyección" → "deberían mantener la constante".
- **Bug real confirmado con los números reales**: Anticipo RP real (jun-ago 2026) se mantenía plano en ~5,2M; apenas entraba a territorio proyectado (sep-2026) saltaba a 66.116.785 — un salto de ~12x sin ningún cambio de headcount (N° seguía en 4). Causa: el split RG/RP de Anticipo (revertido el 18-ago junto con el total, ver 2 entradas arriba) calculaba `RG = 24% × Remuneración_RG` y `RP = Total − RG` (residual). Por álgebra pura, como `Total = 24% × Remuneración_total`, ese residual equivale exactamente a `24% × Remuneración_RP` — es decir, terminaba asumiendo que TODA la planilla RP (~209 personas, Oficina Central/Anexo) pide Anticipo proporcionalmente, cuando en la realidad solo un grupo chico y estable (~4 personas) lo hace.
- **Confirmado comparando contra el Excel real de Finanzas** (mismo archivo `Flujo_Caja_23_06_26.xlsx` de la comparación del 18-ago): su Anticipo RP se mantiene PLANO (~5,5M) en TODOS los meses, incluidos los proyectados — nunca escala con Remuneración_RP. RG en su archivo real absorbe el residual (`RG = Total − RP`), al revés de como estaba el código.
- **Fix real**: nueva `promedioAnticipoRpReal` (refresh.ts, mismo patrón que `promedioRemuneracionReal`/`promedioFiniquitoReal6m` — promedio de los últimos 3 meses reales de `cash_flow_monthly` concepto=`anticipo_rp`). El split ahora ancla RP en ese promedio (`metodoCalculo: "promedio_ultimos_3_meses_reales"`) y RG absorbe el residual (`Total − RP`, `metodoCalculo: "residual_anticipo_total_menos_rp"`) — exactamente al revés de la versión anterior.
- **Verificado**: `npx tsc --noEmit`, `npx vitest run` (125/125) y `npm run build` limpios. Sin test unitario dedicado para este cambio específico (vive dentro de `refresh.ts`, que no tiene archivo de test — mismo criterio que el resto de funciones DB-dependientes de este archivo).
- **Aplicar en**: cuando un concepto se divide en 2 sub-poblaciones (RG/RP) y una fórmula por PORCENTAJE se aplica al total, verificar con datos reales cuál de las 2 sub-poblaciones realmente sigue ese porcentaje y cuál es más bien un grupo chico/estable — el residual SIEMPRE debe caer del lado que de verdad escala con la variable de la fórmula, nunca del lado que es constante por naturaleza de negocio (mismo error de fondo que llevó a intentar y revertir el modelo costo-por-cabeza de Anticipo el 18-ago: no todo concepto financiero escala con headcount o con Remuneración de la misma forma).

### 2026-08-21: Finiquito se simplifica de vuelta a SIEMPRE promedio de 6 meses (se quita la correlación con bajas netas)

- **Pedido del usuario**: "el modelo de finiquito considera el promedio de los últimos 6 meses" — confirmado vía AskUserQuestion que quería SIMPLIFICAR (quitar la rama de correlación con bajas netas de dotación, pedido explícito anterior del 13-ago-2026), no solo confirmar cómo funcionaba.
- **Antes**: `refresh.ts` tenía 2 ramas para el fallback de Finiquito — si la dotación total proyectaba una baja neta ese mes, correlacionaba con `costoPromedioFiniquitoPorBajaNeta` (costo histórico por unidad de baja neta); si no había baja neta o no había histórico calibrable, caía al promedio de 6 meses reales.
- **Fix**: se eliminó la rama de correlación por completo — `finiquitoFallback` ahora es SIEMPRE `promedioFiniquitoReal6m(supabase, mes)` con `metodoCalculo: "promedio_ultimos_6_meses_reales"`, sin condicionales. Se eliminó `costoPromedioFiniquitoPorBajaNeta` (quedó sin uso) y `netVariacionEsteMes` (variable que solo alimentaba esa rama). `dotacionActual`/`dotacionMesAnterior` NO se tocaron — siguen usándose para Remuneración (costo-por-cabeza).
- **Documentación actualizada**: `engine.ts` (comentario de `CashFlowInputs.finiquitoFallback`), `metodologia-contenido.ts` (`CONCEPTOS_METODOLOGIA` y `MOTOR_CAMBIO_MENSUAL`, ambas entradas de Finiquito). Se eliminó de `engine.test.ts` el test que ejercitaba el escenario `correlacionado_bajas_netas_dotacion` — ya no es un escenario real que `refresh.ts` pueda producir (aunque `engine.ts` en sí es agnóstico al string de `metodoCalculo` que recibe, mantener ese test habría estado probando una entrada que ya nunca ocurre en producción).
- **Verificado**: `npx tsc --noEmit`, `npx vitest run` (124/124, bajó 1 por el test eliminado) y `npm run build` limpios.
- **Aplicar en**: cuando el usuario pida simplificar un modelo con ramas condicionales a un solo camino, buscar y eliminar TODA la lógica de la rama descartada (funciones auxiliares que se vuelven dead code, variables que solo alimentaban esa rama, comentarios/documentación en otros archivos que la describen, tests que ejercitan ese escenario) — no solo el `if` visible.

### 2026-08-21: Marca Personal CGS reemplaza Marca Maestra en todo el reporte (rediseño completo, fondo Carbón)

- **Pedido del usuario**: "aplica la marca carlos godoy que está creada como skill para el diseño del flujo de caja" — confirmado vía AskUserQuestion como **reemplazo completo** (fondo Carbón, tema oscuro), no solo una firma discreta ni un híbrido con superficie clara.
- **Alcance confirmado**: `/reporte` (app en vivo), el export HTML autocontenido y el export Excel — más el shell compartido inevitable (`(main)/layout.tsx` nav+footer, `(auth)/login`), ya que ambos envuelven a `/reporte`. **Fuera de alcance deliberado**: `/fuentes` y `/dotacion` (el pedido decía "el diseño del flujo de caja", no toda la app) — como el layout compartido SÍ cambió a fondo Carbón, esas 2 páginas recibieron un fix mínimo de seguridad (fondo blanco explícito en su div raíz) para no quedar con texto oscuro sobre fondo oscuro heredado; su contenido interno sigue sin rediseñar.
- **Verificación WCAG ANTES de aplicar cualquier color** (no después): se calculó contraste real de cada par texto/fondo con un script Node (fórmula de luminancia relativa W3C). Hallazgo clave que cambió el diseño: **Voltio Azul (#1554F3) da solo 3.37:1 sobre Carbón — falla el mínimo de texto (4.5:1)**, aunque sí pasa el umbral de 3:1 para bordes/íconos/UI no-textual. Por eso Voltio se usa en todo el proyecto SOLO como fill/borde/ícono (cabeceras de tabla con texto blanco encima, que sí da 5.27:1) y NUNCA como color de texto pequeño directo sobre Carbón — un error que cometí una vez al escribir `detail-table.tsx` (usé `text-cgs-structure` en la fila "Total Nómina (UF)") y corregí antes de terminar, documentado como ejemplo de por qué verificar cada color, no asumir por el nombre del rol.
- **Colores recalibrados para fondo oscuro** (los de Marca Maestra estaban tuneados para fondo blanco): `--ok` #2e7d32→#4CAF50, `--warn` #7a5209→#F5A623 (ya se había oscurecido el 18-ago para el fondo BLANCO de esa época — recalibrado de nuevo, ahora para Carbón), `--err` #c62828→#EF5350. Efecto colateral encontrado y corregido en el mismo cambio: `alert-panel.tsx` tenía el valor RGB de `--warn` HARDCODEADO en un `rgba()` (workaround de un bug real de Tailwind con opacidad sobre variables CSS en hex, ver Auto-Blindaje anterior) — quedaba desincronizado del nuevo `--warn` si no se actualizaba a mano.
- **Componentes/archivos tocados**: `globals.css` (tokens CGS + shadcn remapeado a oscuro), `tailwind.config.ts` (namespace `cgs.*` + `fontFamily.display/body/mono-cgs`), `layout.tsx` raíz (next/font/google para Unbounded/Manrope/JetBrains Mono, + fix de paso `lang="en"→"es"` y metadata genérica "Forge App" — ambos hallazgos reales de la auditoría `/temple` del 18-ago, resueltos de paso), `(main)/layout.tsx`, `(auth)/login/page.tsx`, `hero-consolidado.tsx`, `cash-flow-area-chart.tsx`, `detail-table.tsx`, `metodologia-calculo.tsx`, `sence-editable-cell.tsx`, `alert-panel.tsx`, `refresh-report-button.tsx`, nuevo `src/shared/ui/cgs-wordmark.tsx` (reemplaza `maestra-logo.tsx`, que queda sin uso pero no se borró — es un asset, no código, no se elimina sin pedirlo explícitamente).
- **HTML exportable (`render.ts`) — decisión revertida a mitad de camino**: intenté cargar Unbounded/Manrope/JetBrains Mono vía `<link>` a Google Fonts (como recomienda la skill para "Web"), pero esto rompe un requisito explícito y TESTEADO del proyecto (`render.test.ts`: "es autocontenido: sin `<link>` ni `<script src>` externos" — el archivo debe poder adjuntarse a un correo y abrirse offline sin depender de red, y muchos clientes de correo bloquean `<link>` de todas formas). Se revirtió a la pila "Office-safe" de la propia skill (Segoe UI Semibold/Segoe UI/Consolas) — mismo criterio que Word/Excel. El wordmark CGS también se generó como texto (`<span>` con subrayado), no como imagen, reemplazando el SVG de Maestra — se actualizó el test correspondiente para buscar `>CGS<` en vez de asumir que cualquier `<svg>` presente era el logo (el gráfico de la página también trae su propio `<svg>`).
- **Excel (`render-excel.ts`)**: la hoja se queda en fondo BLANCO (Excel no tiene un "modo oscuro" real y la propia receta de la skill para Excel asume base clara — cabecera con fill de color, filas alternas claras) — el rediseño ahí es de acento, no de fondo. `FUENTE_MARCA` ("Arial" único) se separó en `FUENTE_TITULO`/`FUENTE_CUERPO` (Segoe UI Semibold/Segoe UI) clasificando cada una de las ~20 celdas con fuente fija por si tenía `bold: true` (título) o no (cuerpo) — reemplazo hecho con un script Node contra números de línea para no perder ninguna ocurrencia. `FILL_HEADER` (navy #003865) → Voltio Azul #1554F3. El logo `maestra-logo.jpg` embebido vía `addImage` se reemplazó por una celda de texto "CGS" en Combustión — la marca CGS es un wordmark tipográfico puro, sin isotipo, así que esto es fiel a la marca, no una simplificación forzada. Imports `readFileSync`/`join`/`dirname`/`fileURLToPath` (usados solo para leer el .jpg) quedaron sin uso, se eliminaron.
- **Verificado**: `npx tsc --noEmit`, `npx vitest run` (124/124) y `npm run build` limpios.
- **Pendiente/no resuelto**: `src/shared/ui/maestra-logo.tsx` y `src/features/report-export/assets/maestra-logo.jpg` quedan en el repo sin ninguna referencia — no se borraron (son assets, no algo que este agente creó en esta sesión); el usuario puede pedir limpiarlos si quiere. `/fuentes` y `/dotacion` quedan con el fix mínimo de fondo blanco — si el usuario quiere extender el tema oscuro ahí también, es trabajo pendiente explícito, no un olvido.
- **Aplicar en**: antes de aplicar CUALQUIER paleta de marca nueva a un fondo oscuro (o cualquier fondo distinto al que la marca documentó como referencia), correr el contraste real de cada color contra ESE fondo específico con una fórmula de luminancia — nunca asumir que "el color X es para texto" porque la guía de marca lo dice así en abstracto; la guía puede (y en este caso lo hizo) documentar un color para un rol que falla WCAG en el fondo real que se está usando.

### 2026-08-21: `payroll_line_items` NO es grano de persona — invalida una premisa de los fixes del 17 y 18-ago (N° de Remuneración RG/RP corregido)

- **Hallazgo/pedido del usuario**, viendo la tabla en vivo: "la dotación de RG y RP histórico está mal y la proyectada también no corresponde" → "todas las dotaciones están mal" → "la sumatoria de dotación no cuadra, debería sumar la dotación de sueldos... revisa excel".
- **Síntoma confirmado**: N° real de Remuneración RG/RP (mayo-julio 2026) mostraba 20/23/23 (RG) y 9/9/9 (RP) — para una Dotación total de 790-841 personas. RG+RP real (≈29-32) no se acercaba NI remotamente al total (790+). En los meses proyectados, el N° saltaba a 662-676 (RG) y 205-209 (RP) — coherente con el total, pero muy distinto del real, generando el mismo "salto ilógico real→proyectado" ya visto 2 veces antes con Anticipo RP.
- **Causa raíz confirmada con una consulta directa a la base (script de solo lectura, borrado después de usar)**: `payroll_source_documents` mostró que el archivo real "Solicitud de Requerimiento remuneracion mayo 2026 RG.xlsx" tiene `filas_procesadas: 20` — el archivo EN SÍ solo trae 20 filas para toda la compañía, desglosadas por sociedad: "MAESTRA CONSTRUCCION S.A." (que sin duda tiene cientos de trabajadores RG) aparece con solo 13 filas. Y en Anticipo (mismo layout de archivo, mismo parser): una fila real tenía `monto: 37.590.000` — matemáticamente imposible que sea el anticipo de 1 sola persona.
- **Conclusión**: `payroll_line_items` NO es grano de persona, es grano de **(sociedad, división/obra)** — es un archivo de REQUERIMIENTO DE TRANSFERENCIA agregado por centro de costo (para que Finanzas pida el giro al banco), no un listado persona por persona. La premisa "1 fila = 1 persona" (documentada como confirmada el 17-ago-2026, nunca vuelta a cuestionar hasta ahora) era incorrecta desde el principio — coincidía por casualidad con números plausibles para Anticipo (pocas divisiones con montos moderados) pero se rompe visiblemente para Remuneración (donde 1 fila por división agrega docenas/cientos de personas).
- **Fix real**: `getDotacionPorConceptoYPeriodo` (dotacion-total.ts) — Remuneración RG/RP **NUNCA MÁS** usa el conteo de `payroll_line_items` (ni para meses reales ni proyectados); siempre usa `rgRp.rg`/`rgRp.rp` (dotación TOTAL × razón histórica en $, vía `dotacionRgRpDelMes`/`proporcionRgHistorica`, ya existente) — esto reconcilia RG+RP con la fila "Dotación (N°)" total, igual que el Excel real de Finanzas (confirmado 18-ago: su N° de Remuneración RG/RP también suma al total de dotación). `CONCEPTOS_CON_DOTACION` se redujo a solo `anticipo_rg`/`anticipo_rp` (ya no hace falta pedir el conteo de remuneración, quedaría sin usar).
- **Lo que NO se tocó, a propósito**: Anticipo RG/RP SIGUE usando el conteo de `payroll_line_items` — es una decisión explícita del usuario (18-ago-2026, vía AskUserQuestion: "Personas que efectivamente cobraron anticipo"), tomada ANTES de este hallazgo. Ese conteo en realidad mide "N° de divisiones/obras con un pago de Anticipo ese mes", no "N° de personas" — la etiqueta ya no es exacta, pero no se cambió el comportamiento sin que el usuario lo pida (queda como pregunta abierta para él, no como decisión unilateral de este agente).
- **Verificado**: `npx tsc --noEmit`, `npx vitest run` (124/124 — sin test dedicado para este cambio, `dotacion-total.ts` no tiene archivo de test, mismo criterio que el resto de funciones DB-dependientes de este archivo) y `npm run build` limpios.
- **Aplicar en**: cuando una tabla se documenta como "grano de X" (persona, división, sociedad), no asumirlo indefinidamente — si un número derivado de esa tabla se ve implausible (muy chico o muy grande para lo que dice representar), verificar el grano real con una fila concreta (¿el monto de esta fila es razonable para 1 unidad del grano asumido?) antes de seguir construyendo lógica encima. La verificación que faltó en 17-ago (correr un valor real y preguntarse "¿esto tiene sentido para 1 persona?") habría encontrado este bug 4 días antes.

### 2026-08-24: El fix del 21-ago para Remuneración RG/RP seguía usando la razón EQUIVOCADA (en $, no en personas) — "el RP está muy dividido"

- **Pedido del usuario**, viendo la tabla en vivo: "además el RP está muy divisado, explica qué está considerando".
- **Investigación con datos reales**: `dotacion_mensual` tiene **43 meses reales** de dotación RG/RP (desde dic-2023) — la razón RG/(RG+RP) en PERSONAS es notablemente estable, ~95-96% en todo ese historial (ej. mayo-2026: 752/(752+38)=95,2%). Pero mayo es el ÚNICO mes de 2026 con fila real en `dotacion_mensual` — junio/julio no tienen fila ahí todavía (gap de sincronización del Excel histórico, no de este fix).
- **Bug real confirmado**: el fix del 21-ago dejó `getDotacionPorConceptoYPeriodo` reconciliando RG+RP con el total (correcto), pero para los meses SIN fila real en `dotacion_mensual` (junio/julio-2026), `dotacionRgRpDelMes` seguía llamando a `proporcionRgHistorica` — que calcula la razón en **$** (RG$/Total$ de `cash_flow_monthly`), no en personas. Como RP (Anexo/Oficina Central) gana en promedio MÁS por persona que RG (Rol General), la razón en $ (~74-78%) es sistemáticamente distinta a la razón real en personas (~95-96%) — el N° de RG/RP proyectado saltaba de forma errática cada vez que pasaba de "mes con dato real" a "mes sin dato real".
- **Fix real**: nueva `proporcionRgHistoricaPersonas` (dotacion-total.ts) — misma estructura que `proporcionRgHistorica`, pero promedia la razón de `dotacion_mensual.rg`/`.rp` (personas), no de `cash_flow_monthly` ($). `dotacionRgRpDelMes` ahora usa esta nueva función para partir el N°. **`proporcionRgHistorica` (la de $) NO se tocó** — sigue usándose en `refresh.ts` para partir el MONTO de Remuneración en RG/RP, donde la razón en $ SÍ es la correcta a propósito (RG y RP tienen sueldos promedio distintos, partir el monto por razón de personas habría sido el error inverso). Son 2 razones DELIBERADAMENTE distintas para 2 propósitos distintos — no intercambiables.
- **Verificado**: `npx tsc --noEmit`, `npx vitest run` (124/124) y `npm run build` limpios.
