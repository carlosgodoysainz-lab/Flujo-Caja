...
- **No gaming.** NUNCA copiar los criterios de eval textualmente al prompt — eso es un estudiante que memoriza el examen: el score sube y la habilidad real no se mueve.
- **Crash tolerance.** Un crash no es el fin. Log it, revert, try different approach.
- **Context management.** NUNCA dejar que outputs largos inunden el contexto — con la ventana llena el loop empieza a olvidar la iteracion anterior y compara contra nada. Si el skill genera archivos, leer solo lo necesario para evaluar.
- **Un principio gana a un score.** Si una optimizacion sube el score pero contradice un principio del Factory OS, se rechaza y se anota por que — un prompt que puntea mejor violando el Golden Path empeora el producto.
