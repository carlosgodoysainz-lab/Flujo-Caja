...
export function calcularBeneficiosDelMes(params: {
  ...
  function agregarPoblacion(poblacion: Poblacion): CashFlowConceptoCalculado {
    const items = lineItems.filter((li) => li.poblacion === poblacion);
    return {
      ...
      // confirmado ese mes, no que los 12 tipos de evento lo sean —
  ...
  return {
    lineItems,
    rg: agregarPoblacion("rg"),
    rp: agregarPoblacion("rp"),
  };
}
